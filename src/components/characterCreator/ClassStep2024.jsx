 ...
 *
 * Edition-specific. Reads exclusively from the dnd5e_2024 adapter
 * (src/data/games/dnd5e_2024/classes.js, sourced from the PHB
 * 2024 SRD). The dispatcher in CharacterCreator.jsx routes here
 * when characterData.gamePack === 'dnd5e_2024'; the legacy
 * ClassStep stays mounted for 2014 characters.
 *
 * What the SRD provides cleanly:
 *   - Hit die, saving throws, primary ability
 *   - Class proficiencies + skill choices
 *   - Multiclass prerequisites + multiclass-grant proficiencies
 *   - Subclass index for navigation
 *
 * What the SRD does NOT provide and is filed as a smell:
 *   - Class playstyle prose / flavour text
 *   - Class portraits / icons
 *   - Per-level features (the `class_levels` field is just a URL
 *     reference; the 2024 SRD has no inline progression). Commit
 *     3 of the 2024 bundle handles this with a hand-authored
 *     features file.
 */

// 2024 subclass-decision level per class. Source: PHB 2024.
// Hand-coded because the SRD doesn't expose the cap directly.
...
// 2024 alignments — shared with 2014 since alignment is a
// system-level concept that hasn't changed. Duplicated here
// rather than imported to keep the 2024 step independent of the
// 2014 step (per the no-fall-through architectural rule).
const ALIGNMENTS = [
...
export default function ClassStep2024({ characterData, updateCharacterData }) {
  ...
  const handleClassSelect = (value) => {
    ...
    updateCharacterData({
      class: value,
      // 2024 features per level land in commit 3; for now the
      // class step seeds the features array with the class id so
      // downstream steps can resolve the right adapter.
      features: [],
      _gamePackClassId: cls?.id || null,
    });
  };
  ...
  return (
    ...
    >
        ...
        {selectedClass && (
          ...
          >
            ...
            {(selectedClass.multiclass?.prerequisites?.length ?? 0) > 0 && (
              <div className="bg-[#37F2D1]/10 rounded-lg p-3 border border-[#37F2D1]/20 text-xs">
      ...
      <div className="space-y-4">
        ...
        <div className="bg-[#1E2430]/90 backdrop-blur-sm rounded-2xl p-6 border border-[#2A3441]">
          <Label className="text-white/70 mb-2 block text-sm uppercase tracking-wide">Portrait</Label>
          {characterData.avatar_url ? (
            <div className="relative h-48 rounded-lg overflow-hidden bg-[#2A3441] mb-3">
              <img
            ...
            <div className="h-48 rounded-lg bg-[#2A3441] flex items-center justify-center mb-3">
        ...
        <div className="bg-[#1E2430]/90 backdrop-blur-sm rounded-2xl p-6 border border-[#2A3441]">
          ...
          <Input
            ...
            className="bg-[#2A3441]/80 border-[#37F2D1]/30 text-white h-12"
          />
          <Label className="text-white/70 mb-2 mt-4 block text-sm uppercase tracking-wide">
            Description
          </Label>
          <Textarea
            ...
            className="bg-[#2A3441]/80 border-[#37F2D1]/30 text-white min-h-[120px]"
          />
        </div>
      </div>
    </motion.div>
