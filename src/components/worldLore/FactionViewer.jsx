...
import { Shield, Eye, EyeOff, Edit, Trash2, TrendingUp, Target, Package, Users as UsersIcon } from "lucide-react";
...
const FACTION_TYPE_COLORS = {
  guild: "#3b82f6",
  noble_house: "#8b5cf6",
  secret_society: "#6366f1",
  religious_order: "#a855f7",
  military: "#ef4444",
  merchant: "#f59e0b",
  criminal: "#dc2626",
  political: "#10b981",
...
export default function FactionViewer({ factions, entries, characters, npcs, canEdit, onSelectFaction, onDeleteFaction }) {
  const [selectedFaction, setSelectedFaction] = useState(null);

  const visibleFactions = canEdit ? factions : factions.filter(f => f.discovered);
  const linkedEntry = selectedFaction && entries?.find(e => e.id === selectedFaction.entry_id);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {visibleFactions.map(faction => {
          const typeColor = FACTION_TYPE_COLORS[faction.type] || FACTION_TYPE_COLORS.other;
          const influenceColor = INFLUENCE_COLORS[faction.influence_level] || INFLUENCE_COLORS.local;
            ...
            <motion.div
              ...
              className={`bg-[#2A3441]/90 backdrop-blur-sm rounded-xl overflow-hidden border cursor-pointer transition-all hover:border-[#37F2D1] ${
                selectedFaction?.id === faction.id 
                  ...
                  : 'border-cyan-400/30'
              } ${!faction.discovered && canEdit ? 'opacity-60' : ''}`}
            ...
            >
              ...
              <div className="p-4">
                <div className="flex items-start justify-between mb-2">
                  <div className="flex-1">
                    <h3 className="text-lg font-bold text-white flex items-center gap-2">
                      {faction.name}
                      {!faction.discovered && canEdit && (
                        <EyeOff className="w-4 h-4 text-gray-500" />
                      )}
                    </h3>
                    {faction.motto && (
                      <p className="text-sm text-gray-400 italic mt-1">"{faction.motto}"</p>
                    )}
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2 mt-3">
                  ...
                  >
                    {faction.type.replace('_', ' ').toUpperCase()}
                  </span>
                  <span 
                    className="text-xs px-2 py-1 rounded font-semibold"
                    style={{ backgroundColor: influenceColor, color: 'white' }}
                  >
                    {faction.influence_level.toUpperCase()}
                  </span>
                </div>
              </div>
            </motion.div>
      ...
      {visibleFactions.length === 0 && (
        <div className="text-center py-12">
          ...
          <p className="text-gray-500">
            {canEdit ? "No factions created yet. Build your world's organizations!" : "No known factions at this time..."}
          </p>
        </div>
      )}
          ...
          <div className="p-6 space-y-6">
            <div className="flex items-start justify-between">
              <div>
                ...
                {selectedFaction.motto && (
                  <p className="text-lg text-gray-400 italic mt-1">"{selectedFaction.motto}"</p>
                )}
                <div className="flex items-center gap-2 mt-3">
                  <span 
                    className="text-sm px-3 py-1 rounded-lg font-semibold"
                    style={{ backgroundColor: FACTION_TYPE_COLORS[selectedFaction.type], color: 'white' }}
                  >
                    {selectedFaction.type.replace('_', ' ').toUpperCase()}
                  </span>
                  <span 
                    className="text-sm px-3 py-1 rounded-lg font-semibold"
                    style={{ backgroundColor: INFLUENCE_COLORS[selectedFaction.influence_level], color: 'white' }}
                  >
                    {selectedFaction.influence_level.toUpperCase()} INFLUENCE
                  </span>
                </div>
              </div>
              {canEdit && (
                <div className="flex gap-2">
                  <Button
                    onClick={(e) => { e.stopPropagation(); onSelectFaction(selectedFaction); }}
                    size="sm"
                  ...
                  <Button
                    onClick={(e) => { e.stopPropagation(); onDeleteFaction(selectedFaction.id); }}
                    size="sm"
                    variant="destructive"
                  >
            ...
            {selectedFaction.description && (
              <div>
                ...
                <p className="text-gray-300 whitespace-pre-wrap">{selectedFaction.description}</p>
              </div>
            )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              {/* Resources */}
              {selectedFaction.resources && selectedFaction.resources.length > 0 && (
                <div>
                  <h4 className="text-sm font-semibold text-gray-400 mb-3 flex items-center gap-2">
                  ...
                  <ul className="space-y-2">
                    {selectedFaction.resources.map((resource, idx) => (
                      <li key={idx} className="text-gray-300 text-sm flex items-start gap-2">
                        <span className="text-[#37F2D1] mt-1">•</span>
                        <span>{resource}</span>
                      </li>
            ...
            {/* Hierarchy */}
            {selectedFaction.hierarchy && selectedFaction.hierarchy.length > 0 && (
              <div>
                <h4 className="text-sm font-semibold text-gray-400 mb-3 flex items-center gap-2">
                ...
                <div className="space-y-2">
                  {selectedFaction.hierarchy.sort((a, b) => parseInt(a.rank) - parseInt(b.rank)).map((h, idx) => (
            ...
            {/* Linked NPCs */}
            {selectedFaction.linked_npcs && selectedFaction.linked_npcs.length > 0 && (
              <div>
                <h4 className="text-sm font-semibold text-gray-400 mb-3 flex items-center gap-2">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {selectedFaction.linked_npcs.map((linkedNpc, idx) => {
            ...
            {/* Key Members (non-NPC) */}
            {selectedFaction.key_members && selectedFaction.key_members.length > 0 && (
              <div>
                <h4 className="text-sm font-semibold text-gray-400 mb-3 flex items-center gap-2">
                  <UsersIcon className="w-4 h-4" />
                  Other Key Members
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {selectedFaction.key_members.map((member, idx) => (
            ...
            {/* Player Reputations */}
            {selectedFaction.player_reputations && Object.keys(selectedFaction.player_reputations).length > 0 && (
              <div>
                <h4 className="text-sm font-semibold text-gray-400 mb-3">Player Reputations</h4>
                <div className="space-y-2">
                  {Object.entries(selectedFaction.player_reputations).map(([charId, score]) => (
