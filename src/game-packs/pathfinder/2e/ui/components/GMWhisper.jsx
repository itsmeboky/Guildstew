...
import { parseAnnotated } from '@/components/glossary/AnnotatedText';

const GMWhisper = ({ children, tone = 'tip' }) => {
  const Icon = tone === 'warning' ? AlertCircle : Lightbulb;
  const body = typeof children === 'string' ? parseAnnotated(children) : children;
  return (
    <div className="relative bg-pf-brass/5 border-l-2 border-pf-brass pl-4 pr-4 py-3 my-4">
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1">
          ...
          <p className="font-body text-sm text-pf-parchment leading-relaxed italic">{body}</p>
        </div>
      </div>
    </div>
  );
