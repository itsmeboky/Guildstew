...
function CharacterCreator() {
  ...
  // Immersive class theming — when the player picks a class, the entire
  // page palette transitions: backgrounds, panel tints, accent ornaments
  // all shift to that class's signature look.
  React.useEffect(() => {
    const cls = CLASSES.find(c => c.id === data.class);
    const root = document.documentElement;
    const arrival = document.getElementById('class-arrival');
    if (cls && cls.theme) {
      root.style.setProperty('--page-bg-1', cls.theme.bg1);
      root.style.setProperty('--page-bg-2', cls.theme.bg2);
      root.style.setProperty('--page-accent', cls.theme.accent);
      root.style.setProperty('--page-accent-deep', cls.theme.accentDeep);
      root.style.setProperty('--aura-tint', cls.color);
      root.style.setProperty('--aura-deep', cls.auraDeep || cls.color);
      root.style.setProperty('--aura-strength', '0.8');
      root.style.setProperty('--class-color', cls.color);
      // Arrival flash — restart animation by toggling class
      if (arrival) {
        arrival.classList.remove('firing');
        void arrival.offsetWidth;
        arrival.classList.add('firing');
      }
    } else {
      // Reset to default warm tavern when no class is picked
      root.style.setProperty('--page-bg-1', '#1A100A');
      root.style.setProperty('--page-bg-2', '#251710');
      root.style.setProperty('--page-accent', '#D4A951');
      root.style.setProperty('--page-accent-deep', '#8C6F3F');
      root.style.setProperty('--aura-tint', '#FF5300');
      root.style.setProperty('--aura-deep', '#5A1A10');
      root.style.setProperty('--aura-strength', '0');
      root.style.setProperty('--class-color', '#FF5300');
      if (arrival) arrival.classList.remove('firing');
    }
  }, [data.class]);
