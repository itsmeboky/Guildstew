      ...
      >
        {!src && (
          <div style={{ textAlign: 'center', color: 'var(--text-faint)', pointerEvents: 'none', padding: 16 }}>
            ...
            <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-dim)' }}>{placeholder || 'Drop or click to upload'}</div>
...
function Stepper({ current, completed, onClick }) {
  return (
    <div className="panel" style={{ padding: '18px 20px 20px', marginBottom: 24 }}>
      <div style={{ display: 'grid', gridTemplateColumns: `repeat(${STEP_DEFS.length}, 1fr)`, gap: 8, marginBottom: 14 }}>
        {STEP_DEFS.map((s, i) => {
          ...
          return (
            <div key={s.id} onClick={() => clickable && onClick(i)}
              style={{ textAlign: 'center', cursor: clickable ? 'pointer' : 'not-allowed', opacity: clickable ? 1 : 0.55 }}>
              <div style={{
                width: 36, height: 36, borderRadius: '50%', margin: '0 auto 8px',
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                fontWeight: 800, fontSize: 14, transition: 'all .2s',
                background: isActive ? 'var(--orange)' : isDone ? 'var(--teal)' : 'var(--bg-3)',
                color: isActive ? 'white' : isDone ? 'var(--bg-1)' : 'var(--text-faint)',
                boxShadow: isActive ? `0 0 0 4px ${'rgba(255,83,0,0.18)'}` : 'none',
              }}>
                {isDone && !isActive ? '✓' : i + 1}
              </div>
              <div style={{
...
// --- Step header (title + subtitle on each step) ---
function StepHeader({ title, subtitle, kicker }) {
  return (
    <div style={{ marginBottom: 24 }}>
      {kicker && (
        <div className="label" style={{ color: 'var(--orange-soft)', marginBottom: 6 }}>{kicker}</div>
      )}
      <h2 className="display" style={{ fontSize: 38, color: 'var(--orange)', marginBottom: 6, lineHeight: 1 }}>{title}</h2>
      {subtitle && <p style={{ color: 'var(--text-dim)', margin: 0, fontSize: 15 }}>{subtitle}</p>}
    </div>
  );
}
